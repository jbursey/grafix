                     ...
                     _In_ int       nCmdShow)
{
    ...
    double frameElapsedTimeMS = 0;
    double secondElapsedTimeMS = 0;
    double timeSinceUpdateMS = 0;
    int frameCount = 0;
    int updateCount = 0;
    ...
    while (isRunning)
    {
        ...
        //--time tracking
        frameElapsedTimeMS = t.GetElapsedTimeMS();
        totalElapsedTimeMS += frameElapsedTimeMS;
        secondElapsedTimeMS += frameElapsedTimeMS;
        timeSinceUpdateMS += frameElapsedTimeMS;

        //--game system update
        while (timeSinceUpdateMS > Grafix::MILLISECONDS_PER_UPDATE)
        {
            //update
            timeSinceUpdateMS -= Grafix::MILLISECONDS_PER_UPDATE;
            updateCount++;
        }
        ...
        if (secondElapsedTimeMS >= 500)
        {
            double fps = framesThisSecond / (1.0 * (secondElapsedTimeMS / 1000.0));
            secondElapsedTimeMS = 0;
            framesThisSecond = 0;
            ...
            debugMsg += L" Total Time: " + std::to_wstring(totalElapsedTimeMS / 1000.0);
            debugMsg += L"\r\n";

            SetWindowText(g_windowHandle, debugMsg.c_str());
        }

...
void TriangleTest::Init(ID3D11Device* device)
{
	...
	vertxBufferDesc.ByteWidth = sizeof(Vertex);
	vertxBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_READ;
	vertxBufferDesc.MiscFlags = 0;
	vertxBufferDesc.StructureByteStride = sizeof(Vertex) * _vertx.size();
	...
	indexBufferDesc.ByteWidth = sizeof(unsigned int);
	indexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_READ;
	indexBufferDesc.MiscFlags = 0;
	indexBufferDesc.StructureByteStride = sizeof(unsigned int) * _indx.size();
...
{	
	unsigned int strides = _vertx.size();
